...
def empty_map(size=96, valid=True, header_names = header_names):
...
def plate_map(file, size=96, valid=True, header_names = header_names):
    ...
    try:   # substitute values w/ new plate map
        ...
        temp = empty_map(size=size, valid=valid, header_names=header_names)   # define an empty plate map
...
def short_map(file, size = 96, valid = True, header_names = header_names):
    ...
    try:
        ...
        finalmap = empty_map(size = size, valid = valid, header_names=header_names)
